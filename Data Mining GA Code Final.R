...
#Creating a neural network with 1 hidden layer and 5 neurons
nn5 <- neuralnet(
  ...
  hidden = 5  # Number of neurons in each hidden layer
)
...
predictions5 <- predict(nn5, newdata = test.norm.df1)

# Convert predictions to binary (1 or 0)
predicted_labels5 <- ifelse(predictions5 > 0.5, 1, 0)

#confusionMatrix(as.factor(test2$Churn > 0.5), as.factor(predicted_labels > 0.5), positive='TRUE')
confusionMatrix(as.factor(predicted_labels5 > 0.5), as.factor(test2$Churn > 0.5), positive='TRUE')

# Optionally, plot the ROC curve
library(pROC)
roc_curve_nn5 <- roc(test2$Churn, as.numeric(predicted_labels5))
plot(roc_curve_nn5, main = "ROC Curve", col = "blue", lwd = 2)
auc_value_nn5 <- auc(roc_curve_nn5)
text(0.8, 0.2, paste("AUC =", round(auc_value_nn5, 2)), col = "blue", cex = 1.2)



#creating an optimal neural network with 1 hidden layer and 8 neurons
nn <- neuralnet(
  Churn1 ~ gender1 + SeniorCitizen1 + Partner1 + Dependents1 + tenure + PhoneService1 + MultipleLines1 + 
    InternetService_DSL1 + InternetService_FiberOptic1 + OnlineSecurity1 + OnlineBackup1 + 
    DeviceProtection1 + TechSupport1 + StreamingTV1 + StreamingMovies1 + Contract_Month1 + 
